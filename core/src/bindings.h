 ...
 * a2600session.h carries the frontend-facing subset of this API.
 *
 * Copyright (C) 2026 Thomas Cherryhomes
 * SPDX-License-Identifier: GPL-3.0-or-later
 */

#ifndef A2600_BINDINGS_H
#define A2600_BINDINGS_H
 ...
 * per a2600session_new (see session.c) -- process-global storage, like
 * gamepad_sdl.c's own pad table, because jzIntv's machine is a process
 * singleton (core/jzintv/a2600_host.h) and there is only ever one session's
 ...
 * (a2600session_key_from_keysym_bound), a physical pad drives at most one
 * side at a time (a2600session_gamepad_assign), so the same button index is
 ...
 * a2600session_target_sysaction), so this also falls back to LEFT's two
 * sysaction slots when `side` isn't LEFT and the per-side scan misses --
 * see the function's own comment. */
a2600session_key_mapping bindings_target_from_button(a2600session_pad_side side,
                                                     a2600session_pad_button button);
...
int bindings_button_is_free(a2600session_pad_side side,
                            a2600session_pad_button button);

/* The gamepad button explicitly bound to each of `side`'s 16 disc positions,
 * indexed by clock position (0 = East, ... see a2600session.h), NONE where
...
int bindings_disc_buttons(a2600session_pad_side side,
                          a2600session_pad_button out[A2600SESSION_DISC_POSITIONS]);

#ifdef __cplusplus
}
#endif

#endif /* A2600_BINDINGS_H */
