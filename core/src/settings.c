/*
 * a2600session settings store: one flat key=value file shared by every
...
static setting_kv *find(struct a2600session *s, const char *key)
{
    setting_kv *kv;
...
static void set_locked(struct a2600session *s, const char *key, const char *val)
{
...
void a2600session_set_str(a2600session *s, const char *key, const char *value)
{
    pthread_mutex_lock(&s->settings_mtx);
...
{
    const char *v = a2600session_get_str(s, key, NULL);
    if (!v || !*v) return def;
    return atoi(v);
}
...
void a2600session_set_int(a2600session *s, const char *key, int value)
{
    ...
    a2600session_set_str(s, key, buf);
}

void a2600session_settings_flush(a2600session *s)
{
    char tmp[A2600_PATH_MAX + 8];
    FILE *fp;
    setting_kv *kv;
    ...
    fp = fopen(tmp, "w");
    if (fp) {
        fprintf(fp, "# FujiNet Go Atari 2600 shared settings\n");
        for (kv = s->settings; kv; kv = kv->next)
            fprintf(fp, "%s=%s\n", kv->key, kv->val);
        fclose(fp);
#if defined(_WIN32)
