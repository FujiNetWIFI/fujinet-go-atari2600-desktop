...
static int load_library_locked(struct a2600session *s)
{
    char errbuf[256];
    if (g_handle) return 0;

    g_handle = a2600_dynlib_open(s->fujinet_lib);
    if (!g_handle) {
        session_set_error(s, "FujiNet library load failed: %s",
                          a2600_dynlib_error(errbuf, sizeof errbuf));
        return -1;
    }
    g_start = (start_runtime_fn)a2600_dynlib_sym(g_handle, "fujinet_desktop_start_runtime");
    g_stop = (stop_runtime_fn)a2600_dynlib_sym(g_handle, "fujinet_desktop_stop_runtime");
    g_last_error = (last_error_fn)a2600_dynlib_sym(g_handle, "fujinet_desktop_last_error_message");
    g_copy_log = (copy_log_fn)a2600_dynlib_sym(g_handle, "fujinet_desktop_copy_recent_log");
...
int fujinet_start(struct a2600session *s)
{
    ...
    if (paths_provision_fujinet(s) != 0) {
        pthread_mutex_unlock(&g_mtx);
        fprintf(stderr, "a2600session: FujiNet runtime unavailable; "
    ...
    {
        char bind[64];
        snprintf(bind, sizeof bind, "127.0.0.1:%d", A2600SESSION_WEBUI_PORT);
        a2600_setenv("FUJINET_WEBUI_BIND", bind);
    }
    ...
    a2600_setenv("FUJINET_RUNTIME_ROOT", s->fujinet_root);

    if (!g_start(s->fujinet_root, s->fujinet_config, s->fujinet_sd,
                 s->fujinet_data, A2600SESSION_BOIP_PORT)) {
...
int fujinet_wait_for_boip(struct a2600session *s, int timeout_ms)
{
    ...
    for (;;) {
        ...
        if (fd >= 0) {
            ...
            addr.sin_port = htons(A2600SESSION_BOIP_PORT);
            addr.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
            int ok = connect(fd, (struct sockaddr *)&addr, sizeof addr) == 0;
            a2600_closesocket(fd);
            if (ok)
                return 0;
        }
