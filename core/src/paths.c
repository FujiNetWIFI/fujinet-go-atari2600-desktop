/*
 * a2600session path layout: XDG config/data directories, the cartridge
 * directory, Stella's own base directory and the FujiNet runtime tree.
 * Transposed from the ColecoVision port's paths.c (same author, same
 * license). There is no ROM directory: the Atari 2600 has no BIOS.
 *
...
#ifndef A2600_DEV_FUJINET_OUT
#define A2600_DEV_FUJINET_OUT ""
#endif
#ifndef A2600_INSTALL_DATADIR
#define A2600_INSTALL_DATADIR ""
#endif
#ifndef A2600_INSTALL_LIBDIR
#define A2600_INSTALL_LIBDIR ""
#endif

#if defined(__APPLE__)
#define A2600_FUJINET_LIB_NAME "libfujinet.dylib"
#elif defined(_WIN32)
#define A2600_FUJINET_LIB_NAME "fujinet.dll"
#else
#define A2600_FUJINET_LIB_NAME "libfujinet.so"
#endif
                        ...
                        const char *win_env, const char *home_suffix)
{
    ...
    (void)xdg_env;
    (void)home_suffix;
    snprintf(dst, dstsz, "%s\\fujinet-go-atari2600", (v && *v) ? v : ".");
#else
    const char *v = getenv(xdg_env);
    (void)win_env;
    if (v && *v) {
        snprintf(dst, dstsz, "%s/fujinet-go-atari2600", v);
    } else {
        const char *home = getenv("HOME");
        snprintf(dst, dstsz, "%s/%s/fujinet-go-atari2600", home ? home : ".",
                 home_suffix);
    }
#endif
}
               ...
               const char *data_dir)
{
    ...
    /* Stella's base dir: its settings file, and the CONFIG client image it
     * spills there for its own createConsole. Trailing separator, as
     * OSystem's getBaseDirectories contract wants. */
    snprintf(s->stella_dir, sizeof s->stella_dir, "%s/stella/", s->data_dir);
    if (mkdir_p(s->stella_dir) != 0) {
        session_set_error(s, "cannot create Stella dir %s", s->stella_dir);
        return -1;
    }
    return 0;
}
...
static int copy_tree(const char *src, const char *dst)
{
    ...
    while ((e = readdir(d)) != NULL) {
        ...
        char from[A2600_PATH_MAX], to[A2600_PATH_MAX];
...
{
    static char dir[A2600_PATH_MAX];
    static int done;
    if (done)
        return dir;
    done = 1;
...
int paths_provision_fujinet(struct a2600session *s)
{
    ...
    char asset_src[A2600_PATH_MAX];
    char probe[A2600_PATH_MAX];
    asset_src[0] = '\0';
    const char *cands[4];
    int nc = 0;
    static char c0[A2600_PATH_MAX], c1[A2600_PATH_MAX];
    if (s->fujinet_runtime_src[0])
        cands[nc++] = s->fujinet_runtime_src;
    if (libdir && *libdir) {
        snprintf(c0, sizeof c0, "%s", libdir);          cands[nc++] = c0;
        snprintf(c1, sizeof c1, "%s/fujinet", libdir);  cands[nc++] = c1;
    }
    cands[nc++] = A2600_INSTALL_DATADIR "/fujinet";
    for (int i = 0; i < nc; i++) {
