// FujiNet Go shadow of Stella's src/common/Variant.hxx (see
// cmake/StageStella.cmake). One change: on Apple platforms the float and
// double string parses use strtof_l/strtod_l instead of std::from_chars,
// which Apple's libc++ marks unavailable before macOS 26 (and the Xcode 16
// SDK does not have for floating point at all). Everything else is
// Stella's, unchanged.
//============================================================================
//
//   SSSS    tt          lll  lll
...
*/
  ...
  #include <cstdlib>
  #include <xlocale.h>
  // Apple's libc++ only gained floating-point std::from_chars in macOS 26
  // (and the Xcode 16 SDK has none at all), so on Apple the float and
  // double parses below go through strtof_l/strtod_l in the "C" locale --
  // the same prefix-parsing, no-locale semantics from_chars has.
  template<typename F>
  inline bool fngoParseFloat(std::string_view sv, F& out)
  {
    static const locale_t cloc = ::newlocale(LC_ALL_MASK, "C", nullptr);
    const std::string tmp(sv);
    char* end = nullptr;
    if constexpr(std::is_same_v<F, float>)
      out = ::strtof_l(tmp.c_str(), &end, cloc);
    else
      out = static_cast<F>(::strtod_l(tmp.c_str(), &end, cloc));
      ...
      if constexpr(F == CacheField::Str) {
        ...
        myCache->f = std::visit([](const auto& v) -> float {
          ...
          if constexpr(std::is_arithmetic_v<T>)  return static_cast<float>(v);
            ...
            auto sv = string_view(v);
          #if defined(__APPLE__)
            return fngoParseFloat(sv, result) ? result : 0.F;
          #else
            const auto [ptr, ec] = std::from_chars(sv.data(), sv.data() + sv.size(), result);
        ...
        myCache->d = std::visit([](const auto& v) -> double {
          ...
          if constexpr(std::is_arithmetic_v<T>)  return static_cast<double>(v);
            ...
            auto sv = string_view(v);
          #if defined(__APPLE__)
            return fngoParseFloat(sv, result) ? result : 0.0;
          #else
