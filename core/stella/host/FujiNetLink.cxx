// FujiNet Go shadow of Stella's src/emucore/fujinet/FujiNetLink.cxx (see
// cmake/StageStella.cmake). One change, in waitFor(): the connect wait also
// selects on the exception set, which is where Winsock reports a refused
// non-blocking connect. Without it every connect attempt on Windows with no
// FujiNet listening waits out CONNECT_TIMEOUT_MS (3 s) on the emulation
// thread: the CONFIG client's screen stays blank and the machine runs slow.
//============================================================================
//
//   SSSS    tt          lll  lll
//  SS  SS   tt           ll   ll
...
// platform split is a handful of aliases rather than a class hierarchy.
  ...
  int waitFor(sock_t s, int64_t remainMs, bool forWrite)
  {
    ...
    // not the write set: without it a refused connection (no fujinet-pc
    // listening) waits out the whole CONNECT_TIMEOUT_MS on Windows. POSIX
    // reports it as writable and the getsockopt(SO_ERROR) that follows
    // sees the error either way, so passing the same set as exceptfds is
    // correct on both.
    fd_set exc = set;
    ...
    return ::select(static_cast<int>(s) + 1, forWrite ? nullptr : &set,
                    forWrite ? &set : nullptr, forWrite ? &exc : nullptr, &tv);
  }
