// FujiNet Go shadow of Stella's src/os/windows/FSNodeWINDOWS.hxx (see
// cmake/StageStella.cmake). One change: the stream constructors take a
// std::filesystem::path instead of a std::wstring. The wstring overloads
// are an MSVC extension; libstdc++ (MSYS2 UCRT64 and mingw-w64, which is
// what builds this app's Windows release) has only the C++17 path overload,
// and that one is portable to both.
//============================================================================
//
//   SSSS    tt          lll  lll
//  SS  SS   tt           ll   ll
...
class FSNodeWINDOWS : public AbstractFSNode
{
  public:
    ...
    std::ifstream openIFStream(std::ios::openmode mode) const override {
      return std::ifstream(std::filesystem::path(_pathW), mode);
    }
    std::ofstream openOFStream(std::ios::openmode mode) const override {
      return std::ofstream(std::filesystem::path(_pathW), mode);
    }
    std::fstream openFStream(std::ios::openmode mode) const override {
      return std::fstream(std::filesystem::path(_pathW), mode);
    }
