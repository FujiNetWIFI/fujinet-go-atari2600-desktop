...
# for the FujiNet Go Atari 2600 desktop app.
#
...
#
# fnconfig.ini is forced to [BOIP] enabled=1 host=127.0.0.1 port=11504
# (A2600SESSION_BOIP_PORT). Note what that means for THIS target: the RS232
...
# in-process emulator's FujiNet cartridge (Stella's CartridgeFUJI, over
# its own FujiNetLink) connects out to it as a plain TCP client. Same
# direction as the CoCo port's DriveWire/Becker link and the Astrocade
# port's cartridge, and the OPPOSITE of the ADAM and Apple II targets, where
# FujiNet is the client. That is what decides startup ordering in
# a2600session_start.
#
...
# Stella has no chdir() of its own today, but the rule below is what keeps
# that from mattering. Root it from the FUJINET_RUNTIME_ROOT env var the
# entry wrapper sets instead. getenv/strlcat are already available (the file uses
...
TARGET_KEY_FILE="${CLONE_DIR}/build/.a2600-target"
...
echo "FujiNet Atari 2600 desktop runtime outputs:"
