/*
 * In-process desktop entry for the FujiNet RS232 runtime (libfujinet.so).
 * Carried across the FujiNet Go desktop family essentially verbatim -- the
 * wrapper has no target-specific logic at all, only the default web admin
 * port below.
 *
...
FUJINET_ENTRY bool fujinet_desktop_start_runtime(
    ...
    /* A2600SESSION_WEBUI_PORT (a2600session.h) -- a high port of this
     * project's own, distinct from every other FujiNet Go desktop target's,
     * so several can run side by side.
     *
     * This default is normally masked: a2600session's fujinet_runtime.c
     * always sets FUJINET_WEBUI_BIND. It still has to be right, because the
     * moment it is not it binds SOMEONE ELSE'S port -- the sibling this was
     * transposed from carries 64003 here, which is the Intellivision port,
     * and would collide with that app rather than fail visibly. */
